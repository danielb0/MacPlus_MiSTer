/*
 ...
 report it (Phase 7 of FLOPPY_WRITE_PLAN.md). It is the one place the
 medium's own sidedness is ever stated: bit 5 set means the track being
 laid down is two-sided, clear means one-sided, and the low five bits are
 the interleave code, which this decoder ignores. Unlike the sector number
 this is checksum-gated: a mis-synced match that slipped through would not
 cost one sector, it would change the geometry of the whole disk, and the
 checksum byte is the next one along anyway.
