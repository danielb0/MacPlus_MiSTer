...
#### Hardware gate (2026-09-17, rbf `MacPlus_c6a0c99f_sdwriter.rbf`) - PASSED, after a day's detour

Two copies of Speedometer 3.23 (409,015 + 41,203 bytes of resource fork, ~910 sectors each) from `boo_.vhd` onto blank 800K images in the internal drive. Both: `PFSW` refused 0, out-of-range 0, landed count moving and then stable; `hfs_integrity.py` EXACT; Machine Records byte-identical. Speedometer's resource fork differed from the source by **one bit**, the same bit in both copies: byte $41 of the fork, 0x65 -> 0x6d.

The detour, kept because the reasoning is reusable: the live source disk (walked through its own catalog on the card) really holds 0x65; the LC's copies of the same file from its own disk hold 0x65; a floppy->HD read-back of a corrected image gave 0x6d from BOTH drives (so not an SDRAM cell in the floppy region, and the write path was not involved); an HD->HD copy with no floppy at all gave 0x6d; MemTest ran 0x55 passes clean; re-copying a 0x6d file left it 0x6d (set-only). What settled it was the header dump: byte $41 is the PAD byte after the file's own name in a record the **File Manager writes into resource-header bytes $30-$7D whenever a resource fork is created** - a "directory copy" (name, long 2, type, creator, flags, twice) from an uncleared buffer. Apple's own words (Technical Note 74): *"The File Manager assumes that the first block of the resource fork of a file will be part of the resource header, and puts information there to aid in scavenging. Note that this means that if you copy a resource file (opened with PBOpenRF), the duplicate may not be exactly like the original."* Reproduced independently by David Cook on 68kMLA (thread 49937, May 2025), who found the range protected against application writes and confirmed the directory copy in the leaked System source. 612 of the large forks on `boo_.vhd` carry the record. The Plus's System 7.1 left 'm' in the pad; the LC's 7.5 left 'e'. `scripts/hfs_fork_diff.py` (new, the fork comparison the gate needs) now compares $30-$7D separately and reports it; with that, both copies are IDENTICAL to the source.

**Lesson:** a Finder copy is not a byte copy of a resource fork. Diff forks with header bytes $30-$7D masked, and before suspecting the core for a single-byte difference, dump the bytes around it - the structure told the story before any experiment did.

**STATUS: implemented, benched, mutation-swept, compiled, HARDWARE-CONFIRMED.** Next: cut `upstream-pr-3`.
